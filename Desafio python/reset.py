...
def buscar_e_analisar_ongs():
    while True:
        ...
        query = input("\nDigite o que deseja procurar (ex: 'ONGs ambientais no Rio Grande do Sul'): ")
        ...
        if not ongs:
            ...
            continuar = input("\nDeseja tentar uma nova pesquisa? (s/n): ").strip().lower()
            if continuar != 's':
                break
            continue
        ...
        # Perguntar ao usuário se deseja fazer uma nova pesquisa
